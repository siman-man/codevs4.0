  ...
  public:
    ...
    void updateTroopsLimit(Unit *unit, bool check = true){
      ...
      if(isEnemyCastelDetected()){
        ...
        if(!isLila(check) && node->enemyUnitCount[BASE] <= 0){
    ...
    void updateUnknownPoint(){
      ...
      //if(!gameStage.castelAttack && (gameStage.targetY > mostDownCoordY || gameStage.targetX > mostRightCoordX)){
    ...
    int directUnitMode(Unit *unit){
      ...
      switch(unit->type){
        case WORKER:
          if(unit->mode == PICKING || (gameStage.incomeResource < createLimit && pickModeCheck(unit))){
    ...
    void updateGameSituation(){
      ...
      if(castel->myUnitCount[BASE] == 0 || (aroundCastelMyUnitCount < aroundCastelEnemyUnitCount+20)){
        ...
        while(it != enemyActiveUnitList.end()){
          ...
          if(!isChokudai() && enemy->type != WORKER && enemy->type != ASSASIN && dist <= DANGER_LINE){
    ...
    int directUnitRole(Unit *unit){
      ...
      if(unit->type == ASSASIN || unit->type == FIGHTER || unit->type == KNIGHT){
        ...
        if(castelDist <= 10){
          ...
          if(attackCount >= 15 && isChokudai(false)){
            gameStage.gameSituation = DANGER;
          }
          if(attackCount >= 20 && isLila(false)){
            gameStage.gameSituation = DANGER;
          }
    ...
    bool canBuildBase(Unit* unit){
      ...
      if(isGrun()){
        ...
        if(isLila()){
          ...
          if(rightDownDist > 70 && myResourceCount <= 1800) return false;
        }
      }
        ...
        if(hitPointY == UNDEFINED && isDie(unit, unit->y, unit->x)){
          ...
          if(unit->type != WORKER){
            ...
            // 基本的に村や拠点は作成しない
              ...
              int dangerPoint = (calcDangerPoint(unit->y, unit->x, 4) >= 3)? 100 : 0;
    ...
    int calcCastelEvaluation(Unit *castel, int operation){
      ...
      // 序盤でどれだけワーカーの数を増やすか
      if(operation == CREATE_WORKER && turn <= 24){
        return 100;
      ...
      switch(unit->mode){
        ...
        case DESTROY:
          if(isEnemyCastelDetected()){
            ...
            int dangerPoint = (calcDangerPoint(unit->y, unit->x, 3) >= 30)? 20 : 0;
            ...
            if(isGrun()){
              ...
              return -20 * abs(diff-enemyCastelDist) - 5 * diffAll - node->receiveDamage[unit->type]/10 - dangerPoint;
            }
            ...
            int diffAll = abs(diffY - diffX);
            int wallDist = calcNearWallDistance(unit->y, unit->x);
            int dangerPoint = (calcDangerPoint(unit->y, unit->x, 3) >= 30)? 10 : 0;

            if(isGrun()){
              return -4 * targetDist + 5 * killCount - 2 * wallDist - node->receiveDamage[unit->type]/10;
            }else{
              return -4 * targetDist - 2 * diffAll - node->receiveDamage[unit->type]/10 - dangerPoint;
            }
          }
          break;
        default:
     ...
     * 危険度を計算する
     */
    int calcDangerPoint(int ypos, int xpos, int eyeRange){
      int dangerPoint = 0;
    ...
    bool isLila(bool check = true){
      ...
      while(it != enemyResourceNodeList.end()){
        ...
        if(check && node->enemyUnitCount[VILLAGE] == 1){
          createLimit = 40;
    ...
    bool isChokudai(bool check = true){
      ...
      if(enemyAI == CHOKUDAI) return true;
