...
const int PINCH           = 7; // 周りに敵がいる状態(村、拠点)
...
const int SAFETY_LINE = 50;         // 自軍の城の周りにこれだけ居れば大丈夫というライン
...
struct Node{
  ...
  int targetCount;                      // このノードを目的地にしているユニットの数
...
int dangerPointList[UNIT_MAX][HEIGHT][WIDTH]; // 危険度
...
class Codevs{
  public:
    ...
    void addMyUnit(int unitId, int y, int x, int hp, int unitType){
      ...
      unit.beforeY            = UNDEFINED;
      unit.beforeX            = UNDEFINED;
     ...
     * 目的地を持っているかどうかのチェック
     */
    bool haveDestination(Unit *unit){
      return (unit->destY != UNDEFINED && unit->destX != UNDEFINED);
    }
    ...
    void removeMyUnit(Unit *unit){
      ...
      if(unit->role == LEADER && unit->troopsLimit > 1){
        if(unit->mode == RESOURCE_BREAK && haveDestination(unit)){
          Node *targetNode = getNode(unit->destY, unit->destX);
          targetNode->targetCount -= 1;
        }
    ...
    void switchLeader(Unit *leader){
      ...
      assert(newLeaderId >= 0 && newLeaderId <= 20000);
    ...
    bool isOccupied(int y, int x){
      ...
      if(node->myUnitCount[WORKER] == 5) return false;
      if(node->myUnitCount[VILLAGE] == 1) return false;

      return (node->timestamp == turn && node->resource && (node->enemyUnitCount[WORKER] > 0 || node->enemyUnitCount[VILLAGE] > 0));
    }
    ...
    void updateResourceNodeData(){
      ...
      while(it != resourceNodeList.end()){
        ...
        // 占領されていたら敵の資源マス
        if(isOccupied(y,x) || rightBottomDist > 100){
          enemyResourceNodeList.insert(y*WIDTH+x);
        }else if(!isOccupied(y,x) && node->timestamp == turn){
          node->targetCount = 0;
    ...
    /*
     * 自分の占領している資源マスで近い場所を調べる
     */
    Coord searchMyOccupiedResourceNode(int ypos, int xpos){
      Coord coord;
      int minDist = MAX_VALUE;
      int dist;
    ...
    Coord searchEnemyOccupiedResourceNode(int ypos, int xpos){
      ...
      while(it != enemyResourceNodeList.end()){
        ...
        if(minDist >= dist && node->targetCount <= 3){
          minDist         = dist;
          coord.y         = y;
          coord.x         = x;
        }
    ...
    void updateTroopsLimit(Unit *unit){
      ...
      if(isEnemyCastelDetected()){
        ...
        int aroundEnemyCount = aroundEnemyUnitCount(unit->y, unit->x, 8).totalCount;

        if(isEnemyCastelSpy() && node->enemyUnitCount[BASE] <= 0){
          unit->troopsLimit = min(unit->troopsLimit, 20);
        }
        if(aroundEnemyCount >= 5){
          unit->troopsLimit = min(unit->troopsLimit, 70);
        }
        if(aroundEnemyCount >= 10){
          unit->troopsLimit = min(unit->troopsLimit, 50);
        }
      }
    }
     ...
     */
    int getNextDirection(Unit *unit){
      ...
      while(!que.empty()){
        ...
        for(int i = 1; i < 5; i++){
          ...
          if(!isWall(ny,nx)){
            int cost = node->stamp + node->cost + dangerPointList[unit->type][ny][nx];
    ...
    void updateUnitMode(){
      ...
      int aroundMyCastelEnemyCount  = aroundEnemyUnitCount(myCastelCoordY, myCastelCoordX, 10).totalCount;
      set<int>::iterator id = myActiveUnitList.begin();
      ...
      while(id != myActiveUnitList.end()){
        ...
        int battleUnitCount = countBattleUnit(unit->y, unit->x, 2);

        if(unit->role == GUARDIAN && gameStage.gameSituation != DANGER && (gameStage.incomeResource >= 30 || battleUnitCount >= 100) && battleUnitCount >= SAFETY_LINE && unit->y == myCastelCoordY && unit->x == myCastelCoordX){
    ...
    /*
     * 新しい小隊を作成する(そのユニットと同じマスにいる戦闘員全てを部下にする)
     */
    void createNewPlatoon(Unit *leader, int mode){
      leader->role = LEADER;
      leader->mode = mode;
      leader->troopsLimit = 1;
      leader->troopsCount = 1;
      ...
      while(id != myActiveUnitList.end()){
        assert(*id >= 0 && *id <= 20000);
        Unit *unit = getUnit(*id);
        int dist = calcManhattanDist(leader->y, leader->x, unit->y, unit->x);

        if(leader->id != unit->id && isBattleUnit(unit) && dist <= 1){
        //if(leader->id != unit->id && isBattleUnit(unit) && leader->y == unit->y && leader->x == unit->x){
          unit->role = COMBATANT;
          unit->leaderId = leader->id;
          leader->troopsCount += 1;
          leader->troopsLimit += 1;
        }
    ...
    void updateVillageBreakerDestination(Unit *breaker){
      ...
      if(coord.y != UNDEFINED && coord.x != UNDEFINED){
        ...
        if(breaker->destY == UNDEFINED && breaker->destX == UNDEFINED){
          ...
          node->targetCount += 1;
        }else if(!isOccupied(breaker->destY, breaker->destX)){
          Node *oldNode = getNode(breaker->destY, breaker->destX);
      ...
      }else if(!haveDestination(breaker)){
        coord = searchMyOccupiedResourceNode(breaker->y, breaker->x);
        ...
        if(coord.y != UNDEFINED && coord.x != UNDEFINED){
          ...
          coord = directTargetPoint(breaker->y, breaker->x, gameStage.targetY, gameStage.targetX);
          breaker->destY = coord.y;
          breaker->destX = coord.x;

          checkMark(breaker->destY, breaker->destX);
    ...
    int directUnitMode(Unit *unit){
      ...
      switch(unit->type){
        ...
        case VILLAGE:
          if(isSafePoint(unit->y, unit->x, 10)){
            return PINCH;
          }else{
            return unit->mode;
          }
          ...
          switch(unit->role){
            case LEADER:
              if(unit->troopsCount >= unit->troopsLimit || gameStage.incomeResource <= 15){
                ...
                if(unit->mode == RESOURCE_BREAK){
                  if(isEnemyCastelDetected() && enemyCastelDist(unit) <= 20){
                    return DESTROY;
                  }else{
                    return RESOURCE_BREAK;
                  }
                }else{
                  return DESTROY;
                }
    ...
    int directUnitMovePriority(Unit *unit){
      if(unit->role == COMBATANT){
        ...
        return 10000 * movePriority[unit->role] - calcManhattanDist(unit->y, unit->x, 90, 90) - calcNearWallDistance(unit->y, unit->x);
      }
    }
    ...
    bool pickModeCheck(Unit *unit){
      ...
      while(it != resourceNodeList.end()){
        ...
        if(!node->rockon && checkMinDist(y, x, dist) && !isDie(unit, y, x) && rightBottomDist >= 100){
          node->rockon = true;
          unit->resourceY = y;
          unit->resourceX = x;
          return true;
    ...
    bool checkMinDist(int y, int x, int minDist){
      ...
      while(id != myActiveUnitList.end()){
        ...
        if(unit->movable && unit->birthday > 20 && unit->mode == SEARCH){
          int dist = calcManhattanDist(unit->y, unit->x, y, x);
     ...
     * リストから排除する。timestampが更新されている場合は、見えてるノードのtimestampを更新する
     */
     ...
     * 戦闘要員かどうかを判定
     */
    bool isBattleUnit(Unit *unit){
      return (unit->type == ASSASIN || unit->type == FIGHTER || unit->type == KNIGHT);
    }
    ...
    bool canBuildBase(Unit* unit){
      ...
      if(isEnemyCastelDetected()){
        if(enemyCastelDist(unit) > 30) return false;
      }
     ...
     * 敵の城との距離
     */
    int enemyCastelDist(Unit *unit){
      assert(isEnemyCastelDetected());
      return calcManhattanDist(unit->y, unit->x, enemyCastelCoordY, enemyCastelCoordX);
    }
    ...
    bool canAssult(Unit *unit, int ypos, int xpos, int range = 2){
      ...
      if(myUnitCount.assasinCount + myUnitCount.fighterCount > min(10, enemyCastelDist(unit)) + (enemyCount.assasinCount + enemyCount.fighterCount/2 + enemyCount.knightCount/3)){
        return true;
    ...
    /*
     * このゲーム全体で有効なユニットの判断を行う
     */
    int checkCurrentBestUnit(){
      int bestUnit = UNDEFINED;
      int assasinCount = gameStage.currentEnemyUnitCount[ASSASIN];
      int fighterCount = gameStage.currentEnemyUnitCount[FIGHTER];
      int knightCount = gameStage.currentEnemyUnitCount[KNIGHT]/3;

      if(fighterCount > assasinCount + knightCount){
    ...
    int checkAdvantageousUnit(int ypos, int xpos, int range = 10){
      ...
      int maxUnitCount = max(max(knightCount/3, fighterCount), assasinCount);

      if(maxUnitCount == assasinCount && maxUnitCount != 0){
      ...
      if(operation == CREATE_BASE && canBuildBase(unit)){
        ...
        if(hitPointY == UNDEFINED && isDie(unit, unit->y, unit->x)){
          ...
          if(gameStage.openedNodeCount == 0){
            ...
            return 100 * myResourceCount + 3 * gameStage.openedNodeCount - 8 * destDist - 3 * stamp - node->cost + 10 * aroundMyUnitDist(unit);
          }
        }else{
          if(unit->type != WORKER){
            if(isEnemyCastelDetected()){
            ...
            }else if(operation == CREATE_VILLAGE && isEnemyCastelDetected() && !isEnemyCastelSpy() && calcManhattanDist(unit->y, unit->x, enemyCastelCoordY, enemyCastelCoordX) <= 10){
              return MAX_VALUE;
            // 基本的に村や拠点は作成しない
            ...
            // 前にいた場所に戻るのはNG
              ...
              int dangerPoint = dangerPointList[unit->type][unit->y][unit->x];
    ...
    int calcKingEvaluation(Unit *king, int operation){
      ...
      if(operation == CREATE_BASE && gameStage.gameSituation == DANGER && castel->myUnitCount[BASE] <= 1 && myResourceCount > 600){
        return 100;
      }else if(operation == CREATE_BASE && gameStage.gameSituation == DANGER && castel->myUnitCount[BASE] <= 2 && myResourceCount >= 800){
        return 150;
      }else if(operation == NO_MOVE){
        return 10;
      }else{
    ...
    int calcVillageEvaluation(Unit *village, int operation){
      ...
      if(operation == CREATE_WORKER && isSafePoint(village->y, village->x, 8) && node->resource && node->myUnitCount[WORKER] <= 5 && village->createWorkerCount <= 5 && income < createLimit){
        ...
        return 110;
      }else if(operation == CREATE_WORKER && isSafePoint(village->y, village->x, 8) && node->resource && centerDist <= 70 && village->createWorkerCount <= 5 && isResourceFull()){
        return 110;
    ...
    int calcBaseEvaluation(Unit *base, int operation){
      if(myResourceCount <= 100 && operation != NO_MOVE && !isEnemyCastelDetected()) return MIN_VALUE;
      if(myResourceCount <= 100 && operation != NO_MOVE && isEnemyCastelDetected() && !isEnemyCastelSpy()) return MIN_VALUE;
      if(gameStage.gameSituation == DANGER && (operation != CREATE_KNIGHT || operation != NO_MOVE)) return MIN_VALUE;
      UnitCount myUnitCount = aroundMyUnitCount(base->y, base->x, 10);
      int bestUnit = (isEnemyCastelSpy())? checkAdvantageousUnit(enemyCastelCoordY, enemyCastelCoordX) : checkAdvantageousUnit(base->y, base->x);

      if(isEnemyCastelDetected()){
        if(operation == CREATE_ASSASIN){
          return 100;
        }else if(operation == bestUnit){
          return 110;
    ...
    int calcGhqEvaluation(Unit *ghq, int operation){
      ...
      int bestUnit = (enemyUnitCount > 10)? checkAdvantageousUnit(ghq->y, ghq->x) : checkCurrentBestUnit();
      Node *node = getNode(ghq->y, ghq->x);

      if((myUnitCount < enemyUnitCount+50) && operation == bestUnit){
        return 100;
    ...
    int calcVillageBreakerEvaluation(Unit *breaker, int operation){
      ...
      if(breaker->destY == gameStage.targetY && breaker->destX == gameStage.targetX){
        return 4 * gameStage.openedNodeCount - 4 * destDist - 2 * node->stamp - node->cost + 10 * topLeftDist - 2 * dangerPoint + 5 * aroundMyUnitDist(breaker);
      }else{
        return 4 * gameStage.openedNodeCount - 4 * destDist - diffAll - 2 * node->stamp - node->cost + 10 * topLeftDist - 2 * dangerPoint + 5 * aroundMyUnitDist(breaker);
      }
    }
    ...
    int calcGuardianEvaluation(Unit *unit, int operation){
      ...
      if(countBattleUnit(unit->y, unit->x) <= 10){
        return 100;
        //return -10 * abs(calcManhattanDist(unit->y, unit->x, myCastelCoordY, myCastelCoordX));
      }else if(countBattleUnit(myCastelCoordY, myCastelCoordX, 1) > 50){
        int diffY   = abs(unit->y - myCastelCoordY);
        int diffX   = abs(unit->x - myCastelCoordX);
        int diffAll = abs(diffY - diffX);

        return -100 * abs(2-calcManhattanDist(unit->y, unit->x, myCastelCoordY, myCastelCoordX)) - 10 * node->myUnitCount[unit->type] - diffAll;
      }else if(node->myUnitTotalCount < 10){
        return -2 * abs(1-calcManhattanDist(unit->y, unit->x, myCastelCoordY, myCastelCoordX));
      }else{
        return -50 * abs(1-calcManhattanDist(unit->y, unit->x, myCastelCoordY, myCastelCoordX)) + penalty;
      }
    }
      ...
      switch(unit->mode){
        ...
        case DESTROY:
          if(isEnemyCastelDetected()){
            ...
            int diff = (enemyCastel->hp < 50000 || enemyCastelDist(unit) <= 11 || canAssult(unit, enemyCastelCoordY, enemyCastelCoordX) || node->enemyUnitCount[BASE] == 0)? 0 : 12;
            ...
            return -20 * abs(diff-enemyCastelDist(unit)) - 5 * diffAll - node->receiveDamage[unit->type]/10;
    ...
    int calcCombatEvaluation(Unit *unit, int operation){
      ...
      int beforeDist = (unit->beforeY != UNDEFINED && unit->beforeX != UNDEFINED)? calcManhattanDist(unit->beforeY, unit->beforeX, leader->y, leader->x) : MAX_VALUE;

      switch(unit->mode){
        case STAY:
          assert(!isWall(leader->y, leader->x));
          if(gameStage.field[leader->y][leader->x].myUnitTotalCount <= limit && unit->y == leader->y && unit->x == leader->x){
            return -100 * calcManhattanDist(unit->y, unit->x, leader->y, leader->x) + penalty;
          }else if(countBattleUnit(leader->y, leader->x, 2) > 50 || countBattleUnit(leader->y, leader->x, 0) > 10){
            int diffY   = abs(unit->y - leader->y);
            int diffX   = abs(unit->x - leader->x);
            int diffAll = abs(diffY - diffX);

            if(beforeDist == 1){
              return 50 - 100 * abs(2-calcManhattanDist(unit->y, unit->x, leader->y, leader->x)) - 10 * node->myUnitCount[unit->type] - diffAll;
            }else if(beforeDist == 2){
              return -100 * abs(2-calcManhattanDist(unit->y, unit->x, leader->y, leader->x)) - 10 * node->myUnitCount[unit->type] - diffAll;
            }else{
        ...
        case DESTROY:
          return value;
          if(isEnemyCastelDetected()){
            if(enemyCastelDist(unit) <= 2){
              return (100 - enemyCastelDist(unit)) + penalty;
      ...
      while(id != enemyActiveUnitList.end()){
        ...
        if(enemy->timestamp == turn){
          if(enemy->type == ASSASIN){
            ...
            addDangerPoint(enemy->y, enemy->x, ASSASIN, 4, -1); 
            addDangerPoint(enemy->y, enemy->x, FIGHTER, 3, 1); 
            addDangerPoint(enemy->y, enemy->x, KNIGHT, 4, 10); 
            addDangerPoint(enemy->y, enemy->x, WORKER, 3, 3); 
          }else if(enemy->type == KNIGHT){
            addDangerPoint(enemy->y, enemy->x, ASSASIN, 3, 1); 
            addDangerPoint(enemy->y, enemy->x, FIGHTER, 4, -3); 
            addDangerPoint(enemy->y, enemy->x, KNIGHT, 3, 1); 
            addDangerPoint(enemy->y, enemy->x, WORKER, 3, 1); 
          }else if(enemy->type == WORKER){
            addDangerPoint(enemy->y, enemy->x, ASSASIN, 5, -2); 
            addDangerPoint(enemy->y, enemy->x, WORKER, 2, 1); 
          }else if(enemy->type == VILLAGE){
            addDangerPoint(enemy->y, enemy->x, KNIGHT, 4, -1); 
          }
        }
     ...
     *       ypos: 基準となるy座標
     *       xpos: 基準となるx座標
     *   unitType: 対象となるユニットのタイプ
     *      range: 危険度を更新する範囲
     *      point: 危険度
     */
    void addDangerPoint(int ypos, int xpos, int unitType, int range, int point){
      map<int, bool> checkList;
      queue<cell> que;
      que.push(cell(Coord(ypos, xpos), 0));
      ...
      while(!que.empty()){
        ...
        dangerPointList[unitType][coord.y][coord.x] += point;
     ...
     * 指定したマスの戦闘要員の数を調べる
     */
    int countBattleUnit(int ypos, int xpos, int range = 0){
      map<int, bool> checkList;
      queue<cell> que;
      ...
      while(!que.empty()){
        ...
        UnitCount unitCount = aroundMyUnitCount(coord.y, coord.x, 0);
        battleUnitCount += unitCount.assasinCount + unitCount.fighterCount + unitCount.knightCount;
